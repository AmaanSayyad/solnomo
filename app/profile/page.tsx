...
export default function ProfilePage() {
    ...
    // Normalize referral code display: bynomo-/binomo- -> solnomo-
    const displayReferralCode = referralCode
      ? referralCode.replace(/^bynomo-/, 'solnomo-').replace(/^binomo-/, 'solnomo-')
      : null;
    ...
    useEffect(() => {
        if (username) {
            setNewUsername(username.replace('.solnomo', ''));
        }
    }, [username]);

    const handleUpdateUsername = async () => {
        if (!address || !newUsername) return;
        const formattedUsername = `${newUsername.toLowerCase()}.solnomo`;
    ...
    if (!isConnected) {
        return (
            ...
            >
                ...
                <h1 className="text-2xl font-black text-white uppercase tracking-widest mb-4">SOLNOMO Registry</h1>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-6">
                            ...
                            >
                                <div className="space-y-5">
                                    ...
                                    <button onClick={() => window.open(`https://twitter.com/intent/tweet?text=Join SOLNOMO: ${displayReferralCode}`, '_blank')} className="w-full py-4 bg-white text-black rounded-xl text-[9px] font-black uppercase tracking-[0.2em] hover:bg-gray-200 transition-all">
                                        Share Access
                                    </button>
                                </div>
                            </motion.section>
