...
export default function ReferralPage() {
    ...
    // Normalize display: show solnomo- prefix (legacy DB may have bynomo- or binomo-)
    const displayCode = referralCode
        ? referralCode.replace(/^bynomo-/, 'solnomo-').replace(/^binomo-/, 'solnomo-').replace(/^solnomo-/, 'solnomo-')
        : null;

    useEffect(() => {
        if (displayCode && typeof window !== 'undefined') {
            setReferralLink(`https://solnomo.fun/?ref=${displayCode}`);
        }
    }, [displayCode]);
    ...
    const shareOnX = () => {
        const text = encodeURIComponent(`Trade with millisecond precision on @Solnomo! 🚀\n\nJoin using my referral link and earn rewards:`);
                    ...
                    >
                        <div className="space-y-4">
                            ...
                            <p className="text-white/30 text-sm font-bold uppercase tracking-widest leading-relaxed max-w-xl">
                                Expand the SOLNOMO neural network. Earn 10% from every trade made by your referred nodes.
                            </p>
                        </div>
                    </motion.div>
                </header>
