...
export const createProfileSlice: StateCreator<ProfileState> = (set, get) => ({
    username: (typeof window !== 'undefined' && localStorage.getItem('solnomo_username')) || null,
    accessCode: (typeof window !== 'undefined' && localStorage.getItem('solnomo_access_code')) || null,
        ...
        try {
            ...
            if (data) {
                ...
                if (typeof window !== 'undefined') {
                    if (data.username) localStorage.setItem('solnomo_username', data.username);
                    if (data.access_code) localStorage.setItem('solnomo_access_code', data.access_code);
                }
            } else {
                set({ accessCode: '' });
            }
