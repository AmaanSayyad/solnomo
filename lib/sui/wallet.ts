...
import { useConnectWallet, useCurrentWallet, useCurrentAccount, useDisconnectWallet } from '@mysten/dapp-kit';
...
export function useWalletConnection() {
   ...
   * 
   * Error handling:
   * - User rejection: Thrown when user cancels connection
   * - Wallet not found: Thrown when no wallet extension is installed
   * - Network issues: Thrown when connection times out or fails
   * 
   * Requirements: 1.4, 14.1
   */
  const connect = async () => {
    try {
      // If there's already a current wallet, connect to it
      if (currentWallet.currentWallet) {
        connectWallet(
          { wallet: currentWallet.currentWallet },
          {
            onSuccess: () => {
              ...
              logInfo('wallet', 'connect_success', { walletName: currentWallet.currentWallet?.name });
            },
            onError: (error) => {
              console.error('Wallet connection failed:', error);
              logWalletError('connect_failed', error, { walletName: currentWallet.currentWallet?.name });
              throw handleWalletError(error);
            }
          }
        );
      } else {
        // Otherwise, just trigger the connect mutation without specifying a wallet
        ...
        connectWallet(
          {} as any,
          {
            onSuccess: () => {
              console.log('Wallet connected successfully');
              logInfo('wallet', 'connect_success', {});
            },
            onError: (error) => {
              console.error('Wallet connection failed:', error);
              logWalletError('connect_failed', error, {});
              throw handleWalletError(error);
            }
          }
        );
